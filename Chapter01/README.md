...
- 탄력적(elastic): 부하 증가를 감지하면 computing resource를 자동으로 추가.
  - 수동으로 확장하는 시스템이 더 간단하고 운영상 예상치 못한 일이 더 적다. (추후 6장에서 다룸)

stateless 서비스를 다수 장비에 배포하는 것은 쉬움.
하지만 stateful 데이터 시스템을 분산 설치하는 것은 아주 많은 복잡도가 추가 발생.
따라서 필요 전까지는 단일 노드에 scaling up하면서 데이터베이스 유지하는것이 최근까지의 통념.

범용적이고 모든 상황에 맞는one-size-fits-all 확장 architecture는 없음. 다음 요소들을 고려해 설계.
- 읽기/쓰기 양
- 저장 데이터 양
- 데이터 복잡도
- 응답시간 요구사항
...
주요 동작이 무엇이고 잘 하지 않는 동작이 무엇인지 가정. 즉, 부하 매개변수 설정. 가정이 잘못되면 노력은 헛수고 심하면 역효과.

도메인에 특화되어도 보통 익숙한 패턴의 나열로 architecture를 구축.

## 유지보수성

레거시 소프트웨어를 직접 만들지 않게끔 소프트웨어를 설계할 때의 원칙
- 운용성(operability): 운영팀이 시스템을 원활하게 운영할 수 있게 쉽게 만들어라
- 단순성(simplicity): 시스템에서 복잡도를 최대한 제거해 새로운 엔지니어가 시스템을 이해하기 쉽게 만들어라.
- 발전성(evlovability): 엔지니어가 이후에 시스템을 쉽게 변경할 수 있게 하라. 그래야 요구사항 변경과 같은 예기치 않은 사용 사례를 적용하기가 쉽다. 유연성extensibility + 수정가능성modifiability + 적응성plasticity.

### 운용성: 운영의 편리함 만들기

> 좋은 운영은 종종 나쁜 소프트웨어의 제약을 피하는 대안이 될 수 있다. 하지만 좋은 소프트웨어라도 나쁘게 운영할 경우 작동을 신뢰할 수 없다.

자동화할 수 있으면 자동화 해야 함. 지속해서 원활하게 작동하려면 운영팀이 필수. 

- 시스템 상태를 모니터링하고 상태가 좋지 않다면 빠르게 서비스 복원

- 시스템 장애, 성능 저하 등 문제의 원인을 추적

- 보안 패치를 포함해 소프트웨어와 플랫폼을 최신 상태로 유지

- 다른 시스템이 서로 어떻게 영향을 주는지 확인해 문제가 생길 수 있는 변경 사항을 손상이 발생하기 전에 차단

- 미래에 발생 가능한 문제를 예측해 문제가 발생하기 전에 해결(capacity)
...
좋은 운영성이란 동일하게 반복되는 테스트를 쉽게 수행하게끔 만드는 것.

- 좋은 모니터링으로 런타임 동작과 시스템 내부의 가시성 제공

- 표준 도구를 이용해 자동화와 통합을 위한 우수한 지원 제공

- 개별 장비 의존성 회피. 유지보수를 위해 장비를 내리더라도 시스템 전체에 영향을 주지 않고 계속 운영 가능해야 함

- 좋은 문서와 이해하기 쉬운 운영 모델 제공.(X하면 Y발생)

- 만족할 만한 기본 동작을 제공하고 필요할 때 기본값을 재정의할 수 있는 권한 부여

- 적절한 자기회복self-healing을 지원하고 필요에 따라 관리자가 시스템 상태를 수동으로 제어할 수 있게 함

- 예측 가능하게 동작하고 예기치 않은 상황을 최소화함

### 단순성: 복잡도 관리

복잡도의 수렁에 빠진 커다란 진흙 덩어리big ball of mud. 예산과 일정이 초과. 버그 생길 위험도 큼.
...
단순하게 만들어야 함. 우발적 복잡도(accidental complexity)를 줄여야 함 → 추상화. 
깔끔하고 직관적인 인터페이스로 세부 구현을 숨김. 좋은 추상화는 큰 시스템의 일부를 잘 정의되고 재사용 가능한 구성 요소로 추출할 수 있게 함.

### 발전성: 변화를 쉽게 만들기

시스템은 끊임없이 변함. 애자일agile 작업 패턴으로 변화에 적응. TDD와 refactoring은 너무 작은 단위에 초점을 맞춤.

대규모 데이터 시스템 수준에서 민첩성/발전성을 높이는 방법에 대해서 추후 다른 장에서 설명할 예정.
